...
Plantilla.prototype.mediaDeEdad = function() {
	const players = [];
	for (let player of tit) {
		players.push(player);
	}
	...
	const media = players.reduce((accummulator, currentValue, index, array) => {
    let suma;

    /*if (acumulador instanceof Jugador) {
      suma = acumulador.edad + valorActual.edad;
    } else {
      suma = acumulador + valorActual.edad;
    }*/

    suma = (acumulador.edad || acumulador) + valorActual.edad;
    
    if (indiceActual === arry.length - 1){
      return suma / arry.length;
    }
    return suma;
